...
@interface MGKeyBoardSymbolView ()
...
@implementation MGKeyBoardSymbolView
{
    float space_x;//按钮竖间距
    float space_y;//按钮行间距
...
- (void)setupSymbolsButton
{
    ...
    for (NSInteger i=0; i<1; i++)
    {
        ...
        {
            UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
            btn.frame = CGRectMake(2*space_x+btn_width/2+(space_x+btn_width)*j, (i+2)*btn_height+3*space_y, btn_width, btn_height);
            [self addSubview:btn];
            [array addObject:btn];
        }
    }
...
- (void)setupSymbolsDeleteButton
{
    ...
    self.delButton.frame = CGRectMake(10*space_x+8*btn_width+btn_width/2, 2*btn_height+3*space_y, 1.5*btn_width, btn_height);
...
- (void)setupSymbolsSwitchButton
{
    ...
    self.switchABCButton.frame = CGRectMake(self.frame.size.width-1*space_x-switchButtonWidth, 3*btn_height+4*space_y, switchButtonWidth, btn_height);
    [self.switchABCButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [self.switchABCButton setTitleColor:[UIColor blackColor] forState:UIControlStateHighlighted];
    self.switchABCButton.titleLabel.font = [UIFont systemFontOfSize:16];
    [self.switchABCButton setTitle:@"ABC" forState:UIControlStateNormal];
    [self.switchABCButton setBackgroundImage:[UIImage imageWithColor:kBackColor] forState:UIControlStateNormal];
    [self.switchABCButton setBackgroundImage:[UIImage imageWithColor:kHightLightColor] forState:UIControlStateHighlighted];
    self.switchABCButton.layer.masksToBounds = YES;
    self.switchABCButton.layer.cornerRadius = kCornerRadius;
...
- (void)showHighlightImg:(NSString *)highImage  button:(UIButton *)button isLabel:(BOOL)isLabel
{
    ...
    if (isLabel) {
        ...
        dLabel.font = [UIFont systemFontOfSize:23];
